...
![deploymentviewNuc (3)](https://user-images.githubusercontent.com/24452907/81227711-c47c0900-8fe4-11ea-9f74-6d70abbb8088.png)
...
```shell
$ git clone https://github.com/AbhinathK/NUC-Optimized-HoloRepository-2020
...
As the system comprises multiple separate components, use docker-compose to locally start all of them at once.

Notes: 
- To successfully start the Accessor, you need to provide the relevant configuration in `./HoloStorageAccessor/config.env` (see the sub-component's `README` for more information).
- To succesfully setup the local FHIR server , you need to complete configuration steps (see the sub-component's `README` for more information)
...
```shell
$ docker-compose -f deployment.yml up holostorage-accessor holorepository-ui-server
```
